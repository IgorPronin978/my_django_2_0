    ...
    def validate_json_data(self, data):
        ...
        for item in data:
            ...
            if category_name not in existing_categories:
                errors.append(
                    f"В новости '{title}' несуществующая категория '{category_name}'. Впишите одну из этих категорий в файл: {', '.join(existing_categories)}")
            for tag_name in tags_names:
                if tag_name not in existing_tags:
                    errors.append(
                        f"В новости '{title}' несуществующий тег '{tag_name}'. Впишите один из этих тегов в файл: {', '.join(existing_tags)}")
        return errors